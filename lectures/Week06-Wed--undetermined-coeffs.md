...
- we calculate 
  \begin{align*}
  & (D^2 -2D -3)[k_1 + k_2e^{-2t}] \\
  =& k_1(D^2 -2D -3)[1] + k_2(D^2 -2D -3)[e^{-2t}] & \text{by linearity}\\
  =& -3k_1 + k_2e^{-2t}((D-2)^2
  -2(D-2) -3)[1] & \text{by the exponential shift formula} \\
  =& -3k_1 + k_2e^{-2t}(D^2 - 4D + 4 -2D +4 -3)[1] &  \\  
  =& -3k_1 + k_2e^{-2t}(D^2 - 6D + 5)[1] &  \\    
  =& -3k_1 + 5k_2e^{-2t} &
  \end{align*}
  
- we now solve for $k_1,k_2$ in the equality $-3k_1 + 5k_2e^{-2t} = 1 + e^{-2t}$.
...
- notice that in this case the *form* of the *simplified guess* for the
  particular solution "looks like" $E(t)$. That doesn't *always* happen, as the next example shows.
...
- note that $e^t\sin(t)$ is a sol to a homog linear ODE
...
- conclude: the smallest operator $P(D)$ for which $P(D)[e^t\sin(t)] = 0$ is $$A(D) = D^2 -2D  + 2.$$
...
- and a simplified guess for the required particular solution to
  ...
  2D^2 + 2D)x=0$ -- i.e by eliminating $$1, \quad e^t\cos(t), \quad e^t\sin(t).$$
